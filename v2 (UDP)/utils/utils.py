...
# Demandes de parole: [(cid, addr, socket), ...]
HOST_SPEAK_REQUESTS = []
HOST_SPEAK_REQUESTS_LOCK = threading.Lock()
...
def _handle_control_message(app, msg):
    if msg == "SPEAKER:1":
        ...
        _log_success(app, "Parle mon cochon !")
    elif msg == "SPEAKER:0":
        _update_speaker_status(app, False)
        _log_grey(app, "En écoute...")
        # Réactiver le bouton de demande quand on perd la parole
        app.call_from_thread(app.reset_speak_request_button)
    elif msg == "SPEAK_REJECTED":
        _log_error(app, "Votre demande de parole a été refusée")
        # Réactiver le bouton pour pouvoir renvoyer une demande
        app.call_from_thread(app.reset_speak_request_button)
    elif msg == "SPEAK_ACCEPTED":
        _log_success(app, "Votre demande de parole a été acceptée!")
    elif msg == "RESET_SPEAK":
        _log_info(app, "Demandes de parole réinitialisées")
        app.call_from_thread(app.reset_speak_request_button)
...
def mic_worker(sock, app):
    ...
    try:
        while True:
            ...
            with HAUT_PARLEUR:
                if app.is_speaker and app.mic_active:
                    try:
...
def _perform_handshake(sock, app):
    ...
    send_message(sock, 0x01, f"PRENOM:{app.prenom}")
    ptype, data = recv_message(sock)
...
def broadcast(data, exclude_id=None):
    ...
    with HOST_CLIENTS_LOCK:
        for sock, _, cid, _ in HOST_CLIENTS:
            if cid != exclude_id:
                try:
                    sock.sendall(pkt)
                except Exception:
...
def set_speaker_host(new_id, app):
    ...
    if new_id == -1:
        ...
        app.add_log("[bold grey] Micro fermé[/]")
    else:
...
def add_speak_request(cid, addr, sock, app):
    """Ajoute une demande de parole à la liste."""
    with HOST_SPEAK_REQUESTS_LOCK:
        # Vérifier que la demande n'existe pas déjà
        if not any(c == cid for c, _, _ in HOST_SPEAK_REQUESTS):
            HOST_SPEAK_REQUESTS.append((cid, addr, sock))
            app.call_from_thread(app.refresh_demandes_table,
                                 list(HOST_SPEAK_REQUESTS))
            app.call_from_thread(
                app.add_log, f"[yellow]📢 Demande de parole du Client {cid}[/]")


def accept_speak_request(cid, app):
    """Accepte une demande de parole et octroie la parole au client."""
    with HOST_SPEAK_REQUESTS_LOCK:
        HOST_SPEAK_REQUESTS[:] = [(c, a, s)
                                  for c, a, s in HOST_SPEAK_REQUESTS if c != cid]

    # Envoyer la confirmation au client
    with HOST_CLIENTS_LOCK:
        for sock, _, client_id, _ in HOST_CLIENTS:
            if client_id == cid:
                try:
                    send_message(sock, 0x01, "SPEAK_ACCEPTED")
                except Exception:
                    pass
                break

    # Octroyer la parole
    set_speaker_host(cid, app)
    app.add_log(f"[bold green] Parole acceptée pour Client {cid}[/]")
    app.refresh_demandes_table([])


def reject_speak_request(cid, app):
    """Refuse une demande de parole."""
    with HOST_SPEAK_REQUESTS_LOCK:
        HOST_SPEAK_REQUESTS[:] = [(c, a, s)
                                  for c, a, s in HOST_SPEAK_REQUESTS if c != cid]

    # Envoyer le refus au client
    with HOST_CLIENTS_LOCK:
        for sock, _, client_id, _ in HOST_CLIENTS:
            if client_id == cid:
                try:
                    send_message(sock, 0x01, "SPEAK_REJECTED")
                except Exception:
                    pass
                break

    app.add_log(f"[bold red] Parole refusée pour Client {cid}[/]")
    app.refresh_demandes_table(list(HOST_SPEAK_REQUESTS))


def reset_all_speak_requests(app):
    """Réinitialise toutes les demandes de parole et notifie les clients."""
    with HOST_SPEAK_REQUESTS_LOCK:
        HOST_SPEAK_REQUESTS.clear()

    # Notifier tous les clients qu'ils peuvent demander à nouveau
    with HOST_CLIENTS_LOCK:
        for sock, _, _, _ in HOST_CLIENTS:
            try:
                send_message(sock, 0x01, "RESET_SPEAK")
            except Exception:
                pass

    app.add_log("[bold yellow] Demandes de parole réinitialisées[/]")
    app.refresh_demandes_table([])
    ...
    try:
        while True:
            ...
            if ptype is None:
                break
            if ptype == 0x01:
                # Messages de contrôle du client
                msg = data.decode() if data else ""
                if msg == "REQUEST_SPEAK":
                    add_speak_request(cid, addr, sock, app)
            elif ptype == 0x02:
                with HOST_SPEAKER_LOCK:
        ...
        with HOST_CLIENTS_LOCK:
            HOST_CLIENTS[:] = [(s, a, i, p)
                               for s, a, i, p in HOST_CLIENTS if i != cid]
        sock.close()
        app.call_from_thread(
...
def server_main(app):
    ...
    try:
        while True:
            ...
            ptype, data = recv_message(sock)
            if ptype != 0x01 or data is None:
                sock.close()
                continue

            msg = data.decode()
            if not msg.startswith("PRENOM:"):
                sock.close()
                continue

            # Extraire le prénom
            prenom = msg.split("PRENOM:", 1)[1]

            cid = client_id_counter
            client_id_counter += 1

            with HOST_CLIENTS_LOCK:
                HOST_CLIENTS.append((sock, addr, cid, prenom))

            send_message(sock, 0x01, f"ID:{cid}")
            app.call_from_thread(
                app.add_log, f"[bold green][+] {prenom} (Client {cid}) connecté depuis {addr[0]}[/]")
            app.call_from_thread(app.refresh_clients_table)
